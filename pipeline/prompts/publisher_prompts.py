...
to Hugging Face Hub with W&B tracking, and persist results to Hippius
...
3. The CRPS is better than any previously published model (or this is the first publish).
   Use `fetch_wandb_runs(limit=5, order="best")` to check.
4. The experiment config is valid (check via `validate_experiment`)

## Procedure

1. **Review the best result** from the Trainer's output.
2. **Check history** — call `fetch_wandb_runs(limit=5, order="best")` to see previously
...
5. **Log metrics** with `log_to_wandb` for tracking.
...
### Historical Analysis
- `fetch_wandb_runs(limit, order)` — query past W&B runs to compare
